...
class Auth0Service {
  boot (options, callback) {
    ...
    this.rbacAdminService = options.bootedServices.rbacAdmin
  ...
  async _fieldFromUserId (userId, field, cache) {
    ...
    await this._addToCache(userId, body.email, body.groups)
  ...
  async _addToCache (userId, email, groups = []) {
    ...
    await this._updateRbac(userId, roles)
  ...
  async _updateRbac (userId, roles) {
    await this.rbacAdminService.clearUserRoles(userId)
    if (roles.length) {
      await this.rbacAdminService.ensureUserRoles(userId, roles)
    }
    await this.rbacAdminService.refreshRbacIndex()
  } // _updateRbac
...
module.exports = {
  ...
  bootAfter: ['caches', 'timestamp', 'auth0GroupMapping', 'rbac']
}
