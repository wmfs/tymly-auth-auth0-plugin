...
class Auth0Service {
  ...
  async emailFromUserId (userId) {
    const email = this.userIdToEmailCache(userId)
    if (email) {
      return email
    }
  ...
  async userIdFromEmail (email) {
    const userId = this.emailToUserIdCache(email)
    if (userId) {
      return userId
    }
  ...
  async groupsFromUserId (userId) {
    const groups = this.userIdToGroupCache(userId)
    if (groups) {
      return groups
    }
  ...
  // //////////////////
  // internal
  emailToUserIdCache(email) { return this.cacheService.get(EMAIL_TO_USER_ID_CACHE_NAME, email) }
  userIdToEmailCache(userId) { return this.cacheService.get(USER_ID_TO_EMAIL_CACHE_NAME, userId) }
  userIdToGroupCache(userId) { return this.cacheService.get(USER_ID_TO_GROUPS_CACHE_NAME, userId) }
